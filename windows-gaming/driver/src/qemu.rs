...
pub fn run(cfg: &Config, tmp: &Path, data: &Path, clientpipe_path: &Path, monitor_path: &Path,
    ...
    qemu.args(&["-enable-kvm",
                ...
                "-device",
                "pcie-root-port,id=root_port,bus=pcie.0",
                "-cpu",
    ...
    for device in cfg.machine.pci_devices.iter() {
        ...
        //This might be a bit toooo hacky
        let rp_address = format!("0x{}.{}", &device.slot[5..7], &device.slot[11..]);
        qemu.args(&["-device", &format!("vfio-pci,host={},multifunction=on,bus=root_port,addr={}", device.slot, rp_address)]);
